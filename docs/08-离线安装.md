...
- 下载所需离线包
  ```
  # 离线安装环境
  curl -LO https://oss.choerodon.com.cn/Kubernetes/Kubernetes-base-amd64.tar
  # 集群运行所需的镜像
  curl -LO https://oss.choerodon.com.cn/Kubernetes/kubernetes-1.30.2-images-amd64.tgz
  # CentOS 7 系统对应软件包
  curl -LO https://oss.choerodon.com.cn/Kubernetes/centos7/kubernetes-1.30.2-centos7-amd64.tgz
  # 克隆脚本
  git clone -b release-1.27 https://mirror.ghproxy.com/github.com/TimeBye/Kubernetes.git
  ```

- **其他系统或ARM64架构点击这里查看所有[离线包](https://oss.choerodon.com.cn/minio/Kubernetes/)**
...
| 192.168.56.15 |    deploy    | CentOS 7.9 | Kubernetes部署工具(ansible) |

**注意：** 以下操作未特殊说明都在 `192.168.56.15` deploy 节点执行。

#### Kubernetes部署工具(ansible) 环境准备

- 上传准备好的 2 个缓存文件至搭建 Kubernetes部署工具(ansible) 环境的服务器上，目录为 `/root` 。

- 准备 Kubernetes部署工具(ansible) 环境
  - 解压离线包
  ```shell
  ...
  - Debian 类系统添加软件源
    ...
    ```
    echo "deb [trusted=yes] file:///root/ packages/" | tee /etc/apt/sources.list.d/Kubernetes.list
    ```
    </details>

- 使用 containerd 运行 Kubernetes 镜像（与 docker 安装二选一）
  <details open>
  ...
  - CentOS 类系统安装 containerd
    ...
    yum install -y --disablerepo=\* --enablerepo=Kubernetes \
      containerd.io
    ```
  ...
  - 配置 containerd
    ...
    [plugins]
      [plugins."io.containerd.grpc.v1.cri"]
        sandbox_image = "registry.custom.local:12480/Kubernetes/pause:3.9"
  ...
  - 启动并加载 Kubernetes 镜像
    ...
    gunzip -c Kubernetes-base-amd64.tar | ctr -n k8s.io images import -
    ```
  - 运行 Kubernetes 镜像
    ```
    ctr -n k8s.io run -d --net-host \
      ...
      --mount type=bind,src=$PWD/Kubernetes,dst=/etc/ansible,options=rbind:ro \
      docker.io/setzero/Kubernetes:base \
      Kubernetes
    ```
  </details>

- 使用 docker 运行 Kubernetes 镜像（与 containerd 安装二选一）
  <details>
  ...
  - 启动并加载 Kubernetes 镜像
    ...
    docker load -i Kubernetes-base-amd64.tar
    ```

  - 运行 Kubernetes 镜像
    ```shell
    docker run -d --restart=always --name Kubernetes \
      -p 12480:12480 \
      -v $PWD/registry:/var/lib/registry \
      -v $PWD/packages:/kubernetes/packages \
      -v $PWD/Kubernetes:/etc/ansible \
      setzero/Kubernetes:base
    ```
  </details>
...
- 推送镜像到已有镜像库（可选项，可跳过）
    ...
    - 如果你不知道这一步可以干什么，请立即跳过
      ...
      # containerd 示例
      ctr -n k8s.io task exec -t --exec-id "shell$RANDOM" Kubernetes bash
      sync-images harbor.custom.io/Kubernetes admin Harbor12345
...
- 编辑变量文件 `./Kubernetes/variables.yaml`，内容如下
  ```yaml
  # 自定义 hosts 记录，方便后期调整 IP；IP 配置为 Kubernetes 镜像运行的服务器的 IP
  # 本例中 192.168.56.15 就是运行 Kubernetes 镜像的服务器
  custom_hosts: 
    "192.168.56.15": 
    - "pkgs.custom.local"
  ...
  # 配置镜像地址
  kube_image_repository: registry.custom.local:12480/Kubernetes
  ```

- 参考 [01-集群安装](./01-集群安装.md#集群规划) 编写 ansible inventory 文件 `./Kubernetes/inventory.ini`。
